...
const DEX_ADDRESSES = {
	...
	JULSWAP: {
		...
		FACTORY: '0x553990F2bba90260ACD95372F6A7E9a01DEe78Fd'
	},
	BAKERYSWAP: {
...
const SMART_ROUTER_METHODS = {
	buyMemeToken: '0x' + ethers.id('buyMemeToken(address,address,address,uint256,uint256)').slice(0, 8),
	swapV3ExactIn: '0x' + ethers.id('swapV3ExactIn(tuple(address,address,address,address,uint256,address,uint256,uint256,uint256,uint256))').slice(0, 8)
};
...
const V3_FILTER = {
	...
	topics: [
		ethers.id('PoolCreated(address,address,uint24,uint24,address)')
	],
	fromBlock: 0,
	toBlock: 'latest'
};
...
async function checkProvider(provider: ethers.JsonRpcProvider): Promise<boolean> {
	try {
...
async function initializeProviders() {
	...
	const httpProviders: ethers.JsonRpcProvider[] = [];
...
let initializedProviders: any = null;
